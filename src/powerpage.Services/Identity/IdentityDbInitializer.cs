...
public class IdentityDbInitializer : IIdentityDbInitializer
{
    ...
    public async Task<IdentityResult> SeedDatabaseWithAdminUserAsync()
    {
        ...
        if (!languages.Any())
        {
            await _languageService.InsertAsync(new Entities.Language { Name = "English", Culture = "en-US" });
            await _languageService.InsertAsync(new Entities.Language { Name = "Arabic", Culture = "ar-OM" });
            await _languageService.InsertAsync(new Entities.Language { Name = "Persian", Culture = "fa-IR" });
            await _languageService.InsertAsync(new Entities.Language { Name = "Turky", Culture = "tr-TR" });
        }
